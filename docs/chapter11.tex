...
campaign, with a companion operations campaign under the same harness policy
defined in Chapter~3. Each listed headline target ran five trials, all listed
targets succeeded, and this chapter applies Chapter~3's interpretation rules when
discussing smaller timing deltas.
...
negative total-time deltas reflect wall-clock noise rather than a real speed-up
from authentication; user-cycle deltas are the more reliable overhead signal and
are positive for every block size (see discussion below).}
\label{tab:auth-overhead-deltas}
\end{table}

The cycle signal is consistent: CTR+HMAC adds user-cycle work at every block
size. Total-time deltas are noisier and close to zero, so cycle deltas are
treated as the more robust overhead indicator.
...
by about 27--32 thousand cycles across the tested payloads, while the percentage
...
for small mode-family deltas. For design conclusions, the report therefore uses
total time mainly for large effects and user cycles for explaining smaller
...
The evaluation succeeded in establishing a reproducible harness and artifact
pipeline, implementing the final authenticated pipeline end-to-end within the
local benchmark/proof workflow, keeping proof verification and mode-family
separation explicit, and supporting the central empirical finding that AES
outperforms LowMC in this zkVM stack with receipt-backed measurements. The main
remaining gaps are unchanged: statistical confidence is still limited for
single-host small timing deltas, legacy supporting snapshots have weaker
provenance than the latest commit-locked report runs, and cross-zkVM replication
is not yet available. Follow-up work should therefore strengthen the benchmark
campaign and artifact provenance before broader claims are made; Chapter~13 gives
that roadmap.
