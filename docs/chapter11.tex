...
The main evaluation source is the May 2026 commit-locked report benchmark
campaign, specifically main run \texttt{20260508-190507Z}, with companion
operations run \texttt{20260508-205918Z} under the same clean-tree harness policy
defined in Chapter~3. Each listed headline target ran five interleaved trials,
all listed targets succeeded, and this chapter applies Chapter~3's interpretation
...
\centering
\reporttablesetup
\begin{tabularx}{\textwidth}{@{}P{0.11\textwidth}LLP{0.16\textwidth}@{}}
\hline
...
backed by aggregate summaries, per-trial records, logs, generated plot data, and
commit-locked \(N=5\) manifests & Achieved for headline report runs \\
...
user-cycle work at every tested block size; no matched wall-clock delta is
significant after correction; full-receipt size increases by 768 bytes & Achieved
for implemented sizes \\
...
\hline
\texttt{aes-ctr-1blk} & 7.2127 & 0.0104 & 7.2231 & 61228 \\
\texttt{aes-ctr-4blk} & 14.4732 & 0.0111 & 14.4842 & 186260 \\
\texttt{aes-ctr-16blk} & 61.6200 & 0.0124 & 61.6324 & 686269 \\
\texttt{aes-ctr-64blk} & 182.4011 & 0.0367 & 182.4378 & 2686216 \\
\hline
\texttt{aes-ctr-hmac-1blk} & 7.2098 & 0.0104 & 7.2202 & 88425 \\
\texttt{aes-ctr-hmac-4blk} & 14.3711 & 0.0111 & 14.3821 & 213704 \\
\texttt{aes-ctr-hmac-16blk} & 61.2811 & 0.0124 & 61.2935 & 714698 \\
\texttt{aes-ctr-hmac-64blk} & 182.5481 & 0.0367 & 182.5848 & 2718641 \\
\hline
\end{tabular*}
...
from 1 through 16 in a separate block-count sweep. Figure~\ref{fig:ctr-trace-rounding-1-to-16} shows the
...
\hline
Blocks & Median total s delta & Median total s delta \% & User-cycle delta & User-cycle delta \% \\
\hline
1 & -0.0029 & -0.04\% & +27197 & +44.42\% \\
4 & -0.1022 & -0.71\% & +27444 & +14.73\% \\
16 & -0.3389 & -0.55\% & +28429 & +4.14\% \\
64 & +0.1471 & +0.08\% & +32425 & +1.21\% \\
\hline
\end{tabular*}
\caption{CTR+HMAC overhead relative to CTR from benchmark results. Near-zero and
negative median total-time deltas reflect wall-clock noise rather than a real
speed-up from authentication; user-cycle deltas are the more reliable overhead
signal and are positive for every block size (see discussion below).}
\label{tab:auth-overhead-deltas}
\end{table}

Table~\ref{tab:auth-overhead-multitrial} adds the multi-trial validity context
for the same final benchmark comparison. The Holm-corrected Welch tests are run
over total-time trial samples; none of the matched CTR versus CTR+HMAC timing
comparisons is statistically significant after correction.
...
\begin{tabularx}{\textwidth}{@{}rrrP{0.15\textwidth}rP{0.16\textwidth}rrr@{}}
\hline
Blocks & \(N\) each & Success & Median total s (CTR/HMAC) & Median delta s & 95\% CI margin s (CTR/HMAC) & Max CV & Holm \(p\) & User-cycle delta \\
\hline
1 & 5 & 10/10 & 7.223 / 7.220 & -0.0029 & 0.036 / 0.033 & 0.40\% & 0.952 & +27197 \\
4 & 5 & 10/10 & 14.484 / 14.382 & -0.1022 & 0.048 / 0.076 & 0.43\% & 0.618 & +27444 \\
16 & 5 & 10/10 & 61.632 / 61.293 & -0.3389 & 0.325 / 0.708 & 0.93\% & 1.000 & +28429 \\
64 & 5 & 10/10 & 182.438 / 182.585 & +0.1471 & 3.185 / 1.084 & 1.40\% & 1.000 & +32425 \\
\hline
\end{tabularx}
\caption{Multi-trial validity summary for final CTR versus CTR+HMAC total-time
comparisons. The adjusted timing tests are non-significant, while user-cycle
deltas are positive for every matched block size.}
\label{tab:auth-overhead-multitrial}
\end{table}

The cycle signal is consistent: CTR+HMAC adds user-cycle work at every block
size. Total-time deltas are noisier and close to zero, and Table~\ref{tab:auth-overhead-multitrial} shows that they do not survive the report's
multi-trial statistical check. Cycle deltas are therefore treated as the more
robust overhead indicator.

The significance of this result is that authentication overhead is visible in
guest-cycle accounting even when wall-clock timing is unstable. User cycles
increase by about 27--32 thousand cycles across the tested payloads, while the
percentage overhead falls as block count grows because fixed authentication work
is amortized over a larger encryption workload. This is the expected direction
for a fixed framing and tag computation cost.

The positive user-cycle deltas do not translate into a reliable wall-clock delta
because the matched CTR and CTR+HMAC targets remain in the same rounded
\(C_{total}\) trace bucket at each tested block size: both 1-block targets use
131072 total cycles, both 4-block targets use 262144, both 16-block targets use
1048576, and both 64-block targets use 3145728. The extra HMAC guest work is
therefore real, but it is not large enough in this dataset to move the final
targets into a larger proof trace bucket.

The near-zero wall-clock deltas do not establish a meaningful timing advantage or
penalty for HMAC at these sizes. They show that wall-clock values are too noisy
for small mode-family deltas under the current single-host \(N=5\) campaign. For
...
In this five-trial dataset, seal size is unchanged by adding HMAC, while full
receipt size increases by a small constant amount.
...
The validity rule used in this chapter follows Chapter~3: large ratios supported
by the \(N=5\) report run can support broad ranking, while small timing
differences need cycle-level support. The main interpretation pattern is
effect-size sensitivity:

\begin{itemize}
\item large ratios, such as LowMC versus AES proving time, are treated as robust
stack-specific findings; the latest LowMC/AES comparison remains significant
after Holm correction (\(p < 0.001\));
\item small total-time differences, especially near-zero HMAC deltas, are treated
...
\begin{itemize}
\item statistical confidence remains limited for single-host small timing deltas;
\item legacy supporting snapshots have weaker provenance than the latest
commit-locked report runs;
