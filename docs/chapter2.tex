...
For the final authenticated path, AES-128 is used in CTR mode under the usual
...
At implementation level, the project does not implement SHA-256 from scratch.
The final authenticated target depends on the RustCrypto \texttt{sha2}
implementation pinned through the RISC Zero-compatible hash repository revision,
so the hash component is a maintained library dependency implementing a vetted
standard rather than new project-specific cryptographic code.
...
of 128 bits to give a larger blind-forgery margin for transcript integrity
without carrying the full 256-bit HMAC output. This does not make the whole
construction a 192-bit-secure system: AES-128 sets the confidentiality level,
while the 192-bit HMAC tag sets the generic blind-forgery bound for integrity.
The two numbers protect different properties, so the report states them
separately rather than implying that the security chain inherits the larger one.
