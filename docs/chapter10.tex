...
encryption key, so the same counter stream is never reused.
\item \textbf{AES-CTR confidentiality:} AES behaves as a secure pseudorandom
function for the counter inputs used by CTR mode \cite{nistsp80038a}.
...
\end{align*}
\item The guest commits public outputs \((\mathrm{aad}, n, c, t, y)\) and the
prover emits a receipt \(\pi\) for the expected guest image.
...
Replace the AES-CTR keystream with a uniformly random string \(r\) of the same
length:
...
Replace the HMAC tag on \(\mathrm{aad} \parallel n \parallel c\) with a random
192-bit value. The tag is already a function of public values after \(G_1\), so
this hop mainly records the assumption that the truncated HMAC output does not
introduce a distinguishable signal about \(b\):
...
Replace the real receipt \(\pi\) with a simulated receipt \(\tilde{\pi}\) over the
same public journal values:

\[
\pi \leftarrow \mathrm{Sim}(\mathrm{aad}, n, c, t, y).
\]
...
plaintext was encrypted. The result is deliberately scoped: it supports the
report's confidentiality and transcript-binding claims for the implemented
benchmark pipeline, but it is not a full AEAD proof or a deployment-level network
security proof.
