...
Guest programs handle deterministic cryptographic computation and journal
commitment. Across the two final AES targets, the core responsibilities are:

\begin{itemize}
\item deserialize keys, nonce/IV, plaintext, and AAD where applicable;
\item run AES-CTR encryption;
\item compute the HMAC tag for authenticated mode over the domain-separated,
length-prefixed frame;
\item commit ciphertext, tag, and public outputs required for verification.
\end{itemize}

Determinism is required so equivalent inputs and image IDs yield reproducible
proof statements.

In practice, the final split means there are separate guest entrypoints for
CTR-only and CTR+HMAC execution. This reduces branch complexity in guest code
and makes each benchmark's proved behavior easier to reason about.

For authenticated mode, keeping encryption and tag generation in the guest is
what allows the proof to bind both outputs to one execution trace.

The guest journal shape is part of the compatibility contract with host code.
Any change to journal fields or serialization format must be synchronized with
host decode logic and benchmark schema expectations.

For CTR+HMAC, the tag input is not an ad-hoc concatenation. The shared
\texttt{compute\_auth\_tag\_192(...)} helper constructs the same framed byte
string in host and guest code: a domain separator, 64-bit big-endian field
lengths, and the AAD, IV/nonce, and ciphertext bytes.
...
runs a guest-side self-check via verify-before-decrypt, and commits outputs to
the journal. The decrypt step here is a benchmark self-check, not a separate
receiver workflow:
...
In authenticated mode, any decryption of returned ciphertext/tag bundles follows
the same verify-before-decrypt contract. Receipt verification remains a separate
host-side check that binds the journal to the expected guest image.
...
A strict rerun should record the repository commit hash before invoking the
harness, because the current artifact schema preserves command and path
provenance but does not embed per-target git revisions directly. This limitation
is revisited in Chapter 12 and prioritized in Chapter 14.
...
wall-clock total time is noisy in a single-trial run.
