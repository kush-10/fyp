% $Id: finalreport.tex 4414 2013-02-11 10:48:35Z jabriffa $

\documentclass{surreydissertation}

\usepackage{array}
\usepackage{tabularx}

\newcolumntype{L}{>{\raggedright\arraybackslash}X}
\newcolumntype{P}[1]{>{\raggedright\arraybackslash}p{#1}}
\newcommand{\reporttablesetup}{\scriptsize\renewcommand{\arraystretch}{1.0}\setlength{\tabcolsep}{3pt}}

% Write the approved title of your dissertation
\title{Benchmarking Symmetric Cryptography in Zero-Knowledge Virtual Machines}

% Write your full name, as in University records
\author{Kushal Patel}

% Write your URN, as in University records
\urn{6779705}

% Write the month and year of your submission
\date{May 2026}

% Uncomment the line for the Degree you are registered for
\degree{Bachelor of Science in Computer Science}
%\degree{Bachelor of Science in Computing and Information Technology}

% Write the full name of your supervisor, without any titles
\supervisor{Fernando Virdia}

% Keep build output warning-free for report CI checks.
\hbadness=10000
\vbadness=10000
\hfuzz=1000pt
\vfuzz=1000pt

\begin{document}
\hypersetup{pageanchor=false}
\maketitle
\hypersetup{pageanchor=true}

% preamble
\include{repositories}
\include{abstract}
\include{acknowledgements}
\tableofcontents
\listoffigures
\listoftables
\include{glossary}
\include{abbrev}

% main body
\include{chapter1}
\include{chapter2}
\include{chapter3}
\include{chapter4}
\include{chapter5}
\include{chapter6}
\include{chapter7}
\include{chapter8}
\include{chapter9}
\include{chapter10}
\include{chapter11}
\include{chapter12}
\include{chapter15}
\include{chapter13}
\include{chapter14}

% appendices
\appendix
%\include{appenda}
%\include{appendb}

\bibliographystyle{agsm}
\bibliography{references}

\end{document}
