...
AES-CTR addresses the first threat under nonce-respecting use. HMAC over the
domain-separated, length-prefixed frame
\(\mathsf{Frame}(\mathrm{aad}, n, c)\) addresses the second under standard
MAC-security assumptions, provided tag verification is enforced before
decryption. Receipt verification against the expected image ID addresses the
third. The fourth is intentionally left outside the implemented cryptographic
payload path and is listed as a transport-layer limitation.

In the implementation, the authenticated frame is not a bare concatenation. It is
the domain separator \texttt{aes-ctr+hmac-sha256-192:v1}, followed by 64-bit
big-endian lengths and field bytes:

\[
\mathsf{Frame}(\mathrm{aad}, n, c) =
\texttt{domain} \parallel |\mathrm{aad}| \parallel \mathrm{aad} \parallel
|n| \parallel n \parallel |c| \parallel c.
\]

This is the byte string passed to HMAC in both host and guest code.
...
\begin{itemize}
\item \textbf{Data authentication:} HMAC over
\(\mathsf{Frame}(\mathrm{aad}, n, c)\) detects tampering in the ciphertext path.
...
the plaintext, authenticated \(\mathsf{Frame}(\mathrm{aad}, n, c)\), and
committed the resulting public transcript. This gives a stronger
receipt-binding claim than proving encryption first and letting the host attach a
tag afterwards.
...
the public tuple contains \((\mathrm{aad}, n, c, t, \pi)\) and the proved relation
includes
...
\item The adversary $\mathcal{A}$ is given access to an encryption oracle
      ...
      and $t = \mathrm{Trunc}_{192}(\mathrm{HMAC}_{k_M}(\mathsf{Frame}(\mathrm{aad}, n, c)))$.
...
Note that the frame includes a domain separator and the lengths of
$\mathrm{aad}$, $n$, and $c$ before their bytes. This is essential: without
length-prefixing, an adversary can take a valid authenticated byte string and
shift bytes between $\mathrm{aad}$ and $c$ to construct a new
$(\mathrm{aad}', c')$ with the same concatenation, producing a valid tag for a
different field boundary. Length-prefixing binds each boundary and prevents this
attack. The INT-CTXT advantage of $\mathcal{A}$ against the construction is
bounded by the HMAC unforgeability advantage plus the blind-forgery term
$q_v / 2^{192}$ for $q_v$ verification queries.
