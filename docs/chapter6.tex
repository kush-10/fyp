...
The AES/HMAC security levels are interpreted by property rather than collapsed
into one headline number. AES-128 determines the confidentiality strength of the
CTR stream cipher, while the 192-bit HMAC truncation determines the generic
blind-forgery bound for transcript integrity. The construction therefore does not
claim 192-bit confidentiality; it uses AES-128 for the measured encryption path
and a longer HMAC tag for a conservative integrity margin.
