...
\texttt{lowmc-r0-optimised} is about \(33.33\times\) slower than
\texttt{aes-r0} on median proving time (484.279 s vs 14.530 s);
\item LowMC optimisation improved local performance materially in supporting
baseline comparisons, but did not reverse the final ranking against AES;
\item adding HMAC increases user-cycle work at every tested block size, while
total-time deltas remain close to zero and noise-sensitive.
\end{itemize}
...
repeatable evaluation workflow. With broader commit-locked target coverage and
...
\centering
\reporttablesetup
\begin{tabularx}{\textwidth}{@{}P{0.08\textwidth}LLP{0.14\textwidth}@{}}
\hline
...
IDs support the report figures and tables with commit-locked \(N=5\) headline
runs & Achieved for headline runs \\
O3 & Explain LowMC/AES behavior & The report links a \(33.33\times\) LowMC/AES
proving-time gap and material LowMC optimisation evidence to VM-level cost
structure & Achieved \\
...
expectation and practical engineering constraint. LowMC initially appeared likely
to outperform AES in a ZK context, but the measured results showed that VM-level
implementation costs can invert circuit-level intuitions; adjusting the design in
response to artifact-backed evidence was therefore the most practically valuable
skill the project reinforced.

The benchmark harness also required more engineering investment than initially
scoped. Reproducibility, artifact traceability, and config-driven trial management
are non-trivial when proving jobs take minutes and machine state changes between
runs, so the dissertation had to make explicit scope decisions --- most notably
single-host headline runs and older single-trial supporting snapshots --- and
report them as methodology rather than hidden limitations.

Finally, constructing the security argument clarified how proof validity and
cryptographic data authenticity answer different questions, and why both layers
are needed in a deployable system. Expressing that decomposition as a structured
game sketch strengthened the contribution beyond a purely empirical benchmark
report and showed where fuller formal treatment would add value.
