...
In verifiable-computation settings, the trust decomposition becomes explicit:
proofs can reduce trust in the executor, but they do not remove the need to
trust the primitive choice, implementation quality, and software supply chain.
The same trust-in-tooling concern applies to the build, library, and
proving-stack chain that produces a zkVM guest binary.
...
One concrete example is privacy-preserving healthcare analytics: a data
provider needs to run a statistical analysis on a sensitive patient cohort and
report aggregated results to a regulator or auditor. Sending the raw cohort to a
third-party compute provider would expose patient data. Instead, the provider
encrypts the inputs inside a zkVM guest program: the program receives the
plaintext, performs the computation, and commits only the aggregated public
outputs to the journal. The resulting receipt gives the auditor cryptographic
evidence that the fixed, inspectable program ran correctly and produced those
outputs, without ever seeing the individual patient records. The key design
question that makes this setting non-trivial is: which cipher should sit inside
the zkVM guest, and how much does it cost to prove that the encryption and
computation ran correctly together?
...
The main limitations are stated here so the results are not overclaimed.

All headline measurements use RISC Zero only and should not be read as claims
about zkVMs in general; a different proving backend could shift performance
rankings. Most final benchmark values come from one trial per target, which is
sufficient for large effect-size comparisons such as LowMC versus AES but not
for small timing differences. Some provenance data, specifically per-target git
commit hashes, is not embedded directly in the benchmark JSON schema, which
limits strict forensic reproducibility. The security argument is a structured
proof sketch rather than a full formal reduction.

Chapter 12 explains the impact of each limitation and Chapter 14 prioritizes
mitigation work.
