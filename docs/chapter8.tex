...
authentication, and proof generation are treated as one auditable computation
pipeline rather than separate unconnected steps. The design objective is to make
every security-relevant output inspectable in the same evidence trail:
configuration, guest execution, receipt verification, and benchmark artifact.
...
In practical terms, this chapter defines the engineering contract used by the
report: if a value appears in evaluation tables, it should be traceable to this
pipeline definition and to one concrete benchmark target in the repository.
...
\item \textbf{One guest target with runtime mode flag:} compact repository layout,
but benchmark IDs could hide whether authentication logic executed.
...
\section{Data Contract and Framing}

Authenticated packets use explicit framing so both host and guest agree on the
exact bytes that are authenticated. The authenticated byte string is the
domain-separated, length-prefixed frame
\(\mathsf{Frame}(\mathrm{aad}, n, c)\), where the implementation serializes the
domain separator, AAD length, AAD bytes, IV/nonce length, IV/nonce bytes,
ciphertext length, and ciphertext bytes. This framing is stable across benchmark
modes and is part of the public contract checked during verification.

Using explicit framing avoids ambiguity bugs where different components hash or
parse data in different orders. The implementation fixes field order,
domain-separation, and length-prefix rules so host-side expectations and
guest-side verification match; if the framing drifts, verification fails
deterministically and the captured benchmark parameters make the mismatch
diagnosable.

\section{Encrypt-then-MAC Construction}

The implemented authenticated mode follows Encrypt-then-MAC with explicit
framing:

\begin{itemize}
\item encryption call: \texttt{encrypt\_ctr(...)};
\item tag call: \texttt{compute\_auth\_tag\_192(...)};
\item authenticated bytes: \(\mathsf{Frame}(\mathrm{aad}, n, c)\);
...
primitive call over framed bytes. This is consistent with the composition model
used in the security analysis chapter.
...
benchmark configuration and run manifests so they can be checked alongside the
receipt-level result.

For reproducibility, each run therefore has two linked evidence layers:

\begin{itemize}
\item cryptographic correctness evidence (journal output and receipt checks);
\item measurement evidence (timings, cycles, trial status, and logs).
\end{itemize}

Treating both layers together avoids a common reporting error where performance
numbers are presented without proof-binding context.
