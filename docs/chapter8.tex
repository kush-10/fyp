...
exact bytes that are authenticated. The authenticated byte string is
\texttt{aad || iv || ciphertext}. This framing is stable across benchmark modes
and is part of the public contract checked during verification.

Using explicit framing avoids ambiguity bugs where different components hash or
parse data in different orders. The implementation fixes field order and byte
concatenation rules so host-side expectations and guest-side verification match;
if the framing drifts, verification fails deterministically and the captured
benchmark parameters make the mismatch diagnosable.
...
\item authenticated bytes: \texttt{aad || iv || ciphertext};
