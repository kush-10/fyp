...
- `run_manifest.json`: full run metadata (config, targets, machine/runtime info)
  plus git/config/tool lock metadata from the current harness.
- `raw/*.json`: one record per target/trial.
- `logs/*.stdout.log` and `logs/*.stderr.log`: captured process output.
- `aggregated.json`: computed medians, means, p95, stddev, confidence intervals,
  IQR/CV, and success metrics.
- `statistical_analysis.json`: report comparison tests and effect sizes when
  generated by `make report-benchmark`.
