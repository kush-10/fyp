...
//! AES-192 implementation with ECB, CTR, and HMAC-SHA256 support.
//!
...
pub use mac::{
    compute_auth_tag_192, hmac_sha256, hmac_sha256_192, verify_auth_tag_192,
...
pub fn encrypt_then_mac(
    ...
    iv: &[u8; 16],
    aad: &[u8],
) -> (Vec<u8>, [u8; HMAC_SHA256_192_TAG_LEN]) {
    let ciphertext = encrypt_ctr(plaintext, enc_key, iv);
...
pub fn verify_then_decrypt(
    ...
    if !verify_auth_tag_192(mac_key, aad, iv, ciphertext, tag) {
        return Err(AesError::InvalidMac);
    }

    Ok(decrypt_ctr(ciphertext, enc_key, iv))
}
...
pub(crate) fn key_expansion(key: [u8; AES_KEY_LEN]) -> KeySchedule {
    const RCON: [Word; 10] = [
        [0x01, 0x00, 0x00, 0x00],
        [0x02, 0x00, 0x00, 0x00],
        [0x04, 0x00, 0x00, 0x00],
...
mod tests {
    ...
    #[test]
    fn encrypt_then_mac_and_verify_then_decrypt_roundtrip() {
        let enc_key = [0x42u8; AES_KEY_LEN];
        let mac_key = [0xA5u8; HMAC_SHA256_KEY_LEN];
        let iv = [0x10u8; 16];
        let aad = b"fyp-aad";
        let plaintext = b"hello authenticated ctr";

        let (ciphertext, tag) = encrypt_then_mac(plaintext, &enc_key, &mac_key, &iv, aad);
        let recovered =
            verify_then_decrypt(&ciphertext, &enc_key, &mac_key, &iv, aad, &tag).unwrap();
        assert_eq!(recovered, plaintext);

        let mut tampered_tag = tag;
        tampered_tag[0] ^= 1;
        assert_eq!(
            verify_then_decrypt(&ciphertext, &enc_key, &mac_key, &iv, aad, &tampered_tag),
            Err(AesError::InvalidMac)
        );
    }
}
